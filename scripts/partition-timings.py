...
def dump_pairwise_dist_mat(Q, part, n_clusts, dirname, graph_size, trialN):
    for clustN in range(n_clusts):
        part_idxs = part_vec == clustN
        Q_part = Q[part_idxs, part_idxs]
        # compute the the pairwise distances
        Q_part_dense = part_utils.compute_pairwise_dist(Q)

        # dump to file
        n_rows = Q_part_dense.shape[0]

        fname = os.path.join(dirname, 'time-' + str(graph_size) + '-' + str(trialN) + '-part-' + clustN + '.csv')
        with open(fname, 'w') as f_out:
            for rowN in range(n_rows):
                q_i = Q_part_dense[rowN, :]
                f_out.write(','.join([str(val) for val in q_i]))
                if rowN < n_rows-1:
                    f_out.write('\n')
...
def main():
    ...
    for graph_size in size_vec:
        for trialN in range(1, n_trials+1):
            ...
            dump_pairwise_dist_mat(Q, part_vec, n_clusts, dirname, graph_size, trialN)
